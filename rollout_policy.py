...
checkpoint = ModelCheckpoint(model_path, monitor='val_acc_top10', save_best_only=True, verbose=1, mode='max')

# early stop參數設定
earlystop = EarlyStopping(monitor='val_acc_top10', patience=6, verbose=1, mode='max')
...
#返回一个编译好的模型
#与删掉的模型相同
#model = load_model(model_path, custom_objects={'acc_top10': acc_top10,'acc_top50': acc_top50, 'loss1': loss1})
#'acc_top10': acc_top10,
##model.compile(loss='sparse_categorical_crossentropy',
##              optimizer=opt,
##              metrics=['sparse_categorical_accuracy',acc_top10,acc_top50,loss1])
...
validation_gen = DataGenerator(X=x_test, y=y_test, batch_size=batch_size, shuffle=True, fp_dim=fp_dim)
